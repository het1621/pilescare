...
export default function BlogContentTab() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.get("/blog-content");
      setData(res.data);
    } catch (err) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await api.patch("/blog-content", data);
      setData(res.data);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-brand-textSecondary mt-1">
            Manage the hero text displayed on the blog index page.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-brand-primary/10 shadow-sm p-6 lg:p-8">
        ...
        <div className="grid md:grid-cols-2 gap-x-6">
          ...
          <Input
            ...
            placeholder="Articles, insights & patient education."
          />
          <div className="md:col-span-2">
            <Textarea
              ...
              placeholder="Evidence-based reading on proctology..."
            />
          </div>
        </div>
      </div>
