...
export default function ServiceContentTab() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.get("/service-content");
      setData(res.data);
    } catch (err) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await api.patch("/service-content", data);
      setData(res.data);
      toast.success("Service page content updated");
    } catch (err) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-brand-textSecondary mt-1">
            Manage the hero text displayed on the services page. (Note: To edit individual services, go to the Services tab).
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-brand-primary/10 shadow-sm p-6 lg:p-8">
        ...
        <div className="grid md:grid-cols-2 gap-x-6">
          <Input
            ...
            placeholder="Treatments & Services"
          />
          <Input
            label="Hero Headline"
            value={data.hero_headline}
            onChange={(v) => update("hero_headline", v)}
            placeholder="Modern proctology, delivered with precision."
          />
          <div className="md:col-span-2">
            <Textarea
              ...
              placeholder="Every treatment we offer prioritises minimal pain..."
            />
          </div>
        </div>
      </div>
