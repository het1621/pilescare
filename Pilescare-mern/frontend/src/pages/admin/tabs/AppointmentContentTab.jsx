...
export default function AppointmentContentTab() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.get("/appointment-content");
      setData(res.data);
    } catch (err) {
      toast.error("Failed to load appointment content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await api.patch("/appointment-content", data);
      setData(res.data);
      toast.success("Appointment content updated");
    } catch (err) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-brand-textSecondary mt-1">
            Manage the hero text displayed on the booking page.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-brand-primary/10 shadow-sm p-6 lg:p-8">
        ...
        <div className="grid md:grid-cols-2 gap-x-6">
          ...
          <Input
            ...
            placeholder="Reserve your private consultation."
          />
          <div className="md:col-span-2">
            <Textarea
              ...
              placeholder="Fill in your details below..."
            />
          </div>
        </div>
        ...
        <div className="mt-6 p-4 rounded-xl bg-blue-50 border border-blue-100 flex items-start gap-3">
          ...
          <p className="text-sm text-blue-900 leading-relaxed">
            <strong>Note on Sidebar Info:</strong> The Clinic Location and Hours shown on the booking page are synced directly from the <strong>Clinic Settings</strong> tab. The Phone and WhatsApp numbers are synced from the <strong>Home Page</strong> tab.
          </p>
        </div>
      </div>
    </div>
