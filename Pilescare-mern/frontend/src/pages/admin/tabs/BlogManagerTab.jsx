...
export function BlogManagerTab() {
  ...
  const save = async () => {
    if (!form.title || !form.excerpt || !form.cover || !form.content_html)
      return toast.error("Title, excerpt, cover and content are required");
    setSaving(true);
    try {
      editing === "new"
        ? await api.post("/blog", form)
        : await api.patch(`/blog/${editing._id}`, form);
      toast.success(
        editing === "new"
      ...
      <div className="mt-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="font-serif text-2xl text-brand-text font-semibold">
            {editing === "new" ? "New Post" : `Edit: ${editing.title}`}
          </h2>
        ...
        <div className="bg-white rounded-2xl border border-brand-primary/10 p-6 lg:p-8 grid md:grid-cols-2 gap-5">
          ...
          <Field label="Cover Image URL *" full>
            ...
            {form.cover && (
              <img
                src={form.cover}
                alt="preview"
                className="mt-3 w-full max-w-sm aspect-video object-cover rounded-xl border border-brand-primary/10"
              />
            )}
          </Field>
          ...
          <Field label="Content HTML *" full>
            <textarea
              rows={14}
              className="m-input resize-y font-mono text-xs"
        ...
        {preview && form.content_html && (
          <div className="bg-white rounded-2xl border border-brand-primary/10 p-8">
            <h3 className="font-serif text-3xl text-brand-text font-medium">
              {form.title}
            </h3>
            {form.cover && (
              <img
                src={form.cover}
                alt=""
                className="mt-5 w-full aspect-video object-cover rounded-2xl"
              />
            )}
            <div
              className="prose-content mt-6"
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(form.content_html),
              }}
            />
    ...
    <div className="max-w-6xl space-y-12 mt-6">
      ...
      <div className="pt-10 border-t border-brand-primary/10">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-serif text-brand-text font-medium">Blog Posts</h2>
            ...
            {posts.map((p) => (
              <li key={p._id} className="p-5 flex items-center gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-brand-primary font-semibold">
                    {p.category}
                  </span>
                  <div className="font-serif text-lg text-brand-text font-semibold truncate">
                    {p.title}
                  </div>
                ...
                <div className="flex gap-1 shrink-0">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="p-2 rounded-lg text-brand-textSecondary hover:bg-brand-subtle"
                  >
                    <Eye size={15} />
                  </a>
                  <button
