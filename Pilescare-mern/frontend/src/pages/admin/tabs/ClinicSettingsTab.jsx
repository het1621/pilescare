...
const FIELDS = [
  {
    key: "clinic_name",
    label: "Clinic Name",
    placeholder: "ProctoCare by Vishva",
  },
  {
    key: "tagline",
    label: "Tagline",
    placeholder: "Advanced Proctology Care…",
  },
  {
    key: "address_line1",
    label: "Address Line 1",
    placeholder: "3rd Floor, Sterling Centre",
  },
  {
    key: "address_line2",
    label: "Address Line 2",
    placeholder: "Race Course Circle",
  },
  {
    key: "landmark",
    label: "Landmark",
    placeholder: "Opp. SBI ATM, near Akota Garden",
  },
  ...
  { key: "pincode", label: "PIN Code", placeholder: "390007" },
  {
    key: "hours",
    label: "Clinic Hours",
    placeholder: "Mon–Sat: 10 AM – 1 PM & 4 PM – 7:30 PM",
    multiline: true,
  },
  {
    key: "contact_email",
    label: "Public Contact Email",
    placeholder: "drvishvapatel6298@gmail.com",
  },
  {
    key: "maps_link",
    label: "Google Maps Share Link (Get Directions button)",
...
export function ClinicSettingsTab() {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.error(
        formatApiError(err.response?.data?.detail) || "Failed to save",
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="mt-6 space-y-6">
      <div className="flex items-start gap-3 bg-brand-secondary border border-brand-primary/10 rounded-xl p-4">
        ...
        <p className="text-xs text-brand-textSecondary leading-relaxed">
          Changes here update the public Contact page and footer instantly. For
          the embedded map, go to{" "}
          <strong>Google Maps → Share → Embed a map</strong> and copy the URL
          from inside <code>src="…"</code>.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-brand-primary/10 p-6 lg:p-8 grid md:grid-cols-2 gap-5">
        {FIELDS.map((f) => (
          <div key={f.key} className={f.multiline ? "md:col-span-2" : ""}>
            ...
            {f.hint && (
              <p className="text-[11px] text-brand-textMuted mt-1.5 leading-snug">
                {f.hint}
              </p>
            )}
          </div>
      ...
      {form.maps_embed_url && (
        <div className="bg-white rounded-2xl border border-brand-primary/10 p-6 lg:p-8">
          <h3 className="font-serif text-xl text-brand-text font-semibold mb-4">
            Map Preview
          </h3>
