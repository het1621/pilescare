...
export default function TestimonialContentTab() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.get("/testimonial-content");
      setData(res.data);
    } catch (err) {
      toast.error("Failed to load testimonial content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await api.patch("/testimonial-content", data);
      setData(res.data);
      toast.success("Reviews page content updated");
    } catch (err) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-brand-textSecondary mt-1">
            Manage the hero text displayed on the reviews/testimonials page.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-brand-primary/10 shadow-sm p-6 lg:p-8">
        ...
        <div className="grid md:grid-cols-2 gap-x-6">
          ...
          <Input
            ...
            placeholder="Real voices, real outcomes."
          />
          <div className="md:col-span-2">
            <Textarea
              ...
              placeholder="Each review comes from a verified patient..."
            />
          </div>
        </div>
      </div>
