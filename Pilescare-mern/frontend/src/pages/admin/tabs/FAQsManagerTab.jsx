...
const CATS  = ["Concerns","Treatments","Appointment","Recovery","Online","General"];
...
export function FAQsManagerTab() {
  ...
  const load=useCallback(async()=>{
    setLoading(true);
    try{const{data}=await api.get("/faqs");setItems(data);}
  ...
  const save=async()=>{
    ...
    setSaving(true);
    try{
      editing==="new"?await api.post("/faqs",form):await api.patch(`/faqs/${editing._id}`,form);
  ...
  const remove=async f=>{
    ...
    try{await api.delete(`/faqs/${f._id}`);toast.success("Deleted");load();}
    catch(err){toast.error(formatApiError(err.response?.data?.detail));}
  };
        ...
        :<ul className="divide-y divide-brand-primary/5">
          {items.map(f=>(
            <li key={f._id} className="p-5 flex items-start gap-5">
              <span className="text-[10px] uppercase tracking-[0.2em] text-brand-primary font-semibold shrink-0 w-24 mt-0.5">{f.category}</span>
              ...
              <div className="flex gap-1 shrink-0">
                <button onClick={()=>startEdit(f)} className="p-2 rounded-lg text-sky-600 hover:bg-sky-50"><Pencil size={14}/></button>
                <button onClick={()=>remove(f)} className="p-2 rounded-lg text-rose-600 hover:bg-rose-50"><Trash2 size={14}/></button>
              </div>
            </li>
          ))}
        </ul>}
