...
export default function ServicesManagerTab() {
  ...
  const save = async () => {
    if (!form.title || !form.summary)
      return toast.error("Title and summary required");
    const payload = {
      ...form,
      ...
      <div className="mt-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="font-serif text-2xl text-brand-text font-semibold">
            {editing === "new" ? "New Service" : `Edit: ${editing.title}`}
          </h2>
          ...
          {form.image && (
            <div className="md:col-span-2">
              <img
                src={form.image}
                alt="preview"
                className="aspect-video w-64 object-cover rounded-xl border border-brand-primary/10"
              />
            </div>
          )}
        </div>
    ...
    <div className="max-w-6xl space-y-12 mt-6">
      ...
      <div className="pt-10 border-t border-brand-primary/10">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-serif text-brand-text font-medium">Services List</h2>
              ...
              <li key={s._id} className="p-5 flex items-center gap-5">
                <div className="w-16 h-16 rounded-xl overflow-hidden bg-brand-secondary shrink-0">
                  {s.image ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-brand-primary text-xs font-semibold">
                      {s.icon}
                    </div>
                  )}
                </div>
...
export const MInputStyles = () => null;
