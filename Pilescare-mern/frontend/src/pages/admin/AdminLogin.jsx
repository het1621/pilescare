...
export default function AdminLogin() {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        if (el) { el.style.transition = "opacity .6s ease, transform .6s ease"; el.style.opacity = 1; el.style.transform = "translateY(0)"; }
      }, 50);
    });
  }, []);
      ...
      <div ref={rootRef} className="relative z-10 w-full max-w-md">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-10 shadow-2xl">
          ...
          <Link to="/" className="flex items-center gap-3 mb-10">
            ...
            <div>
              <div className="font-serif text-xl text-white font-semibold">ProctoCare</div>
              <div className="text-[10px] tracking-[0.2em] uppercase text-white/40 -mt-0.5">by Vishva</div>
            </div>
          </Link>
          ...
          <h1 className="font-serif text-3xl text-white font-semibold">Admin Sign In</h1>
          <p className="text-sm text-white/45 mt-2">Manage appointments, content & settings.</p>

          <form onSubmit={submit} className="mt-8 space-y-5">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-[0.18em] text-white/40 mb-2">Email</label>
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                className="adm-input" placeholder="admin@proctocarebyvishva.com" />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-[0.18em] text-white/40 mb-2">Password</label>
          ...
          <div className="mt-6 text-center">
            <Link to="/" className="text-xs text-white/30 hover:text-white/60 transition-colors">← Back to website</Link>
          </div>
        </div>
      </div>

      <style>{`
        .adm-input{width:100%;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.12);border-radius:.75rem;padding:.75rem 1rem;font-size:.9rem;color:#fff;outline:none;transition:border-color .2s,box-shadow .2s;font-family:inherit}
        .adm-input::placeholder{color:rgba(255,255,255,.25)}
        .adm-input:focus{border-color:rgba(26,91,94,.7);box-shadow:0 0 0 3px rgba(26,91,94,.25)}
      `}</style>
    </div>
  );
}
