...
import AboutManagerTab from "./tabs/AboutManagerTab";
import HomeContentTab from "./tabs/HomeContentTab";
import AppointmentContentTab from "./tabs/AppointmentContentTab";
import api from "../../lib/api";

const TABS = [
  { id: "dashboard", label: "Dashboard", Icon: LayoutDashboard },
  { id: "home", label: "Home Page", Icon: LayoutDashboard },
  { id: "about", label: "About Doctor", Icon: Stethoscope },
  { id: "appointment_content", label: "Booking Settings", Icon: LayoutDashboard },
...
export default function AdminDashboard() {
  ...
  const renderTab = () => {
    switch (tab) {
      ...
      case "home":
        return <HomeContentTab />;
      case "about":
        return <AboutManagerTab />;
      case "appointment_content":
        return <AppointmentContentTab />;
      case "appointments":
        return <AppointmentsTab />;
      case "services":
        return <ServicesManagerTab />;
      case "blog":
        return <BlogManagerTab />;
      case "testimonials":
        return <TestimonialsManagerTab />;
      case "faqs":
        return <FAQsManagerTab />;
      case "settings":
  ...
  return (
    <div
      className="min-h-screen bg-brand-subtle flex"
    ...
    >
      ...
      <>
        ...
        <aside
          className={`fixed top-0 left-0 bottom-0 z-40 w-64 bg-brand-dark flex flex-col
        ...
        >
          ...
          <div className="p-6 border-b border-white/5">
            <Link to="/" className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-brand-primary text-white flex items-center justify-center font-serif text-lg font-semibold">
                P
              </div>
              <div>
                <div className="font-serif text-base text-white font-semibold leading-tight">
                  ProctoCare
                </div>
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            {TABS.map(({ id, label, Icon }) => (
              ...
              >
                ...
                {id === "appointments" && pendingCount > 0 && (
                  <div className="bg-rose-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md min-w-[20px] text-center">
                    {pendingCount}
                  </div>
                )}
              </button>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="bg-white border-b border-brand-primary/8 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-[0_2px_12px_rgba(26,91,94,0.05)]">
          <div className="flex items-center gap-4">
            <button
              ...
              className="lg:hidden p-1.5 rounded-lg text-brand-textSecondary hover:bg-brand-subtle transition-colors"
            >
              <Menu size={20} />
            </button>
            <div>
              <h1 className="font-serif text-xl text-brand-text font-semibold">
                {currentTab?.label}
              </h1>
              <p className="text-xs text-brand-textMuted hidden sm:block">
                ProctoCare Admin Panel
              </p>
            </div>
          </div>
          <Link
        ...
        {/* Tab content */}
        <main className="flex-1 p-6 lg:p-8 overflow-y-auto">{renderTab()}</main>
      </div>
    </div>
  );
}
