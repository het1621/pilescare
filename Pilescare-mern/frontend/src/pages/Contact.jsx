...
const Field = ({ label, children }) => (
  <div>
    <label className="contact-form-label block text-xs uppercase tracking-[0.18em] mb-2">
      {label}
    </label>
    {children}
  </div>
...
export default function Contact() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.error(
        formatApiError(err.response?.data?.detail) || "Failed to send",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  // Live clinic open/closed indicator — Mon–Sat 9am–7pm (IST)
  const now = new Date();
  const dayOfWeek = now.getDay(); // 0=Sun, 6=Sat
  const hour = now.getHours();
  const isClinicOpen = dayOfWeek >= 1 && dayOfWeek <= 6 && hour >= 9 && hour < 19;
  ...
  const defaultMapUrl =
    "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d118106.70010221669!2d73.17308625!3d22.32210265!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x395fc8ab91a3ddab%3A0xac39d3bfe1473fb8!2sVadodara%2C%20Gujarat!5e0!3m2!1sen!2sin!4v1714392472439!5m2!1sen!2sin";
    ...
    <div ref={rootRef} data-testid="contact-page">
      ...
      <section className="bg-brand-dark py-20 lg:py-28">
        <div className="container-page max-w-3xl">
          <span className="ct-hero-txt label-eyebrow !text-brand-accent/80">
            Contact
          </span>
          <h1 className="ct-hero-txt font-serif text-5xl lg:text-7xl text-white font-medium tracking-tight mt-4 leading-[1.04]">
            We'd love to
            <br />
            <em className="text-brand-accent italic">hear from you.</em>
          </h1>
          <p className="ct-hero-txt mt-6 text-white/55 text-lg leading-relaxed">
            Every message is read and responded to personally. Expect a reply
      ...
      <section className="ct-grid container-page py-16 grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-4">
          {/* Email */}
          <a
            href={`mailto:${s.contact_email || "drvishvapatel6298@gmail.com"}`}
          ...
          >
            ...
            <div>
              <div className="text-xs uppercase tracking-[0.2em] text-brand-textMuted mb-1">
                Email
              </div>
          ...
          >
            ...
            <div>
              ...
              <div className="font-serif text-base text-brand-text font-semibold">
                {homeData?.phone_number || "+91 99999 99999"}
              </div>
            </div>
          </a>
          ...
          <div className="ct-info-card card-soft p-6 flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <div className="text-xs uppercase tracking-[0.2em] text-brand-textMuted">Clinic Hours</div>
                <span className={`flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full
                  ${isClinicOpen ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-600"}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${isClinicOpen ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`} />
              ...
              {s.hours ? (
                ...
                <div className="space-y-1.5">
                  {CLINIC_SCHEDULE.map(({ days, time }) => (
                    <div key={days} className="flex justify-between text-sm">
                      <span className="text-brand-textSecondary">{days}</span>
                      <span className={`font-medium ${time === "Closed" ? "text-brand-textMuted" : "text-brand-text"}`}>{time}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="ct-info-card card-soft p-6">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                ...
                {s.clinic_name && (
                  <div className="font-serif text-base text-brand-primary font-semibold">
                    {s.clinic_name}
                  </div>
                )}
                {addr.map((line, i) => (
        ...
        <div className="ct-form-wrap lg:col-span-7">
          {done ? (
            <div
              className="card-soft p-12 text-center h-full flex flex-col items-center justify-center"
            ...
            >
              ...
              <p className="text-sm text-brand-textSecondary mt-3 max-w-xs">
                Thank you, {form.name || "friend"}. We'll get back to you within
                one working day.
              </p>
              <button
                onClick={() => {
            ...
            <form onSubmit={submit} className="card-soft p-8 lg:p-10 space-y-5">
              <h2 className="font-serif text-2xl text-brand-text font-semibold mb-2">
                Send a Message
              </h2>
