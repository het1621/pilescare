  ...
  return (
    <div className="bg-white rounded-2xl border border-brand-primary/10 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={prev}
          className="p-1.5 rounded-lg hover:bg-brand-subtle text-brand-textSecondary transition-colors"
        >
          <ChevronLeft size={16} />
        </button>
        <span className="font-serif text-base font-semibold text-brand-text">
          {MONTHS[month]} {year}
        </span>
        <button
          onClick={next}
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {DAYS.map((d) => (
          <div
            key={d}
            className="text-center text-[10px] font-semibold uppercase tracking-wider text-brand-textMuted py-1"
          >
            {d}
          </div>
        ))}
...
const Field = ({ label, children }) => (
  <div>
    <label className="book-form-label block text-xs uppercase tracking-[0.18em] mb-2">
      {label}
    </label>
    {children}
  </div>
...
export default function BookAppointment() {
  ...
  useEffect(() => {
    api.get("/appointment-content").then(({ data }) => setPageData(data)).catch(() => {});
    api.get("/clinic-settings").then(({ data }) => setSettings(data)).catch(() => {});
    api.get("/home-content").then(({ data }) => setHomeData(data)).catch(() => {});
  }, []);

  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from(".book-hero-text", {
        y: 36,
        opacity: 0,
        duration: 0.9,
        ease: "power3.out",
    ...
    return (
      <div className="container-page py-24" ref={rootRef}>
        <div className="max-w-xl mx-auto card-soft p-12 text-center">
          ...
          <h1 className="font-serif text-3xl text-brand-text font-semibold">
          ...
          <p className="text-brand-textSecondary mt-4 leading-relaxed text-sm">
            ...
            <strong>{confirmation.preferred_date}</strong> at{" "}
            <strong>{confirmation.time_slot}</strong> has been received. Our
            care coordinator will call you on{" "}
            <strong>{confirmation.contact_number}</strong> to confirm.
          </p>
          {confirmation.email && (
            <p className="text-xs text-brand-textMuted mt-3">
              You'll receive an email notification at {confirmation.email} when
    ...
    <div ref={rootRef} data-testid="booking-page">
      ...
      <section className="bg-brand-dark py-16 lg:py-20">
        <div className="container-page text-center max-w-3xl mx-auto">
          <span className="book-hero-text label-eyebrow !text-brand-accent/80">
            {pageData?.hero_eyebrow || "Book An Appointment"}
          </span>
          <h1 className="book-hero-text font-serif text-4xl lg:text-6xl text-white font-medium tracking-tight mt-4 leading-[1.05]">
            {pageData?.hero_headline || "Reserve your private consultation."}
          </h1>
          <p className="book-hero-text mt-5 text-white/55 text-base leading-relaxed max-w-xl mx-auto">
            {pageData?.hero_subheadline || "Fill in your details below. Slots are confirmed by our care coordinator within a few working hours."}
          </p>
        </div>
      </section>
      ...
      <section className="container-page py-16 grid lg:grid-cols-3 gap-10 items-start">
        ...
        <form
          ...
          className="book-form-wrap lg:col-span-2 card-soft p-8 lg:p-10 space-y-6 opacity-100"
        >
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setForm({ ...form, consultation_type: opt.id })
                  }
                ...
                >
                  ...
                  <div
                    className={`text-xs mt-0.5 ${form.consultation_type === opt.id ? "text-white/70" : "text-brand-textMuted"}`}
                  >
                    {opt.sub}
                  </div>
                </button>
                  ...
                  {slots.map((s) => (
                    <button
                      ...
                      className={`rounded-full px-3 py-2 text-xs font-medium transition-all duration-200
                        ${
                          !s.available
                            ? "bg-brand-subtle text-brand-textMuted line-through cursor-not-allowed"
                            : form.time_slot === s.slot
          ...
          <Field label="Additional Notes (optional)">
            <textarea
              ...
              placeholder="Anything you'd like the doctor to know in advance…"
            />
          </Field>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="btn-primary w-full justify-center !py-4 text-base shadow-[0_6px_20px_rgba(26,91,94,0.3)]"
          >
            {submitting ? (
              <>
          ...
          <p className="text-xs text-brand-textMuted text-center">
            By submitting, you agree to be contacted by our care coordinator.
            Your information is fully confidential.
          </p>
        </form>
          ...
          {[
            {
              icon: "🕒",
              title: "Clinic Hours",
              body: settings?.hours || "Mon–Sat: 10 AM – 1 PM & 4 PM – 7:30 PM\nSunday: Closed",
            },
            {
              icon: "📍",
              title: "Clinic Location",
              body: settings?.address_line1 ? `${settings.address_line1}\n${settings.city || ""} ${settings.pincode || ""}` : "3rd Floor, Sterling Centre\nRace Course Circle, Vadodara 390007",
            },
            {
              icon: "📞",
              title: "Quick Call",
              body: `${homeData?.phone_number || "+91 99999 99999"}\nFor urgent queries or same-day slots`,
            },
            {
              icon: "💬",
              title: "WhatsApp",
              body: "Message us any time for faster responses", // Or dynamic if needed, but WhatsApp CTA is fairly standard
            },
          ].map(({ icon, title, body }) => (
            <div key={title} className="book-side-card card-soft p-5">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-brand-secondary flex items-center justify-center text-xl shrink-0">
                  {icon}
                </div>
                <div>
                  <div className="font-serif text-base font-semibold text-brand-text">
                    {title}
                  </div>
                  <p className="text-xs text-brand-textSecondary mt-1 leading-relaxed whitespace-pre-line">
                    {body}
                  </p>
                </div>
              </div>
