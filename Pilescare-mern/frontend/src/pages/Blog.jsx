...
const FALLBACK = [
  { _id:"1", slug:"what-are-piles",       category:"Symptoms",   read_time:"6 min read", title:"What Are Piles? Symptoms, Causes, and When to See a Doctor", excerpt:"Piles (haemorrhoids) affect 1 in 3 adults at some point. Learn to recognise the signs early and understand why timely treatment makes all the difference.", cover:"https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=800&q=80" },
  { _id:"2", slug:"laser-vs-surgery",     category:"Treatment",  read_time:"8 min read", title:"Laser Piles Treatment vs Traditional Surgery: What You Need to Know", excerpt:"Modern laser procedures have changed the game for piles treatment. Here's an evidence-based comparison of laser versus conventional surgical approaches.", cover:"https://images.unsplash.com/photo-1579684385127-1ef15d508118?w=800&q=80" },
  { _id:"3", slug:"fissure-fistula-diff", category:"Education",  read_time:"5 min read", title:"Fissure vs Fistula: Understanding the Difference", excerpt:"Anal fissure and fistula are often confused. This guide explains what each condition is, how they feel, and why treatment is completely different.", cover:"https://images.unsplash.com/photo-1559757148-5c350d0d3c56?w=800&q=80" },
  { _id:"4", slug:"diet-after-piles",     category:"Recovery",   read_time:"4 min read", title:"Best Diet After Piles Surgery: A Complete Recovery Guide", excerpt:"What you eat in the two weeks after laser piles surgery determines how fast and smoothly you recover. Here's a practical, easy-to-follow meal plan.", cover:"https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=800&q=80" },
  { _id:"5", slug:"pilonidal-sinus",      category:"Education",  read_time:"5 min read", title:"Pilonidal Sinus: Causes, Symptoms, and Modern Treatment Options", excerpt:"Pilonidal sinus is common in young adults and frequently misunderstood. Learn about EPSIT — the minimally invasive treatment that avoids open wounds.", cover:"https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&q=80" },
  { _id:"6", slug:"online-consult-tips",  category:"Tips",       read_time:"3 min read", title:"How to Prepare for Your Online Proctology Consultation", excerpt:"Getting the most out of a video consultation requires a little preparation. Here's exactly what to have ready so Dr. Vishva can help you effectively.", cover:"https://images.unsplash.com/photo-1587825140708-dfaf72ae4b04?w=800&q=80" },
];
...
export default function Blog() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(".blog-hero-txt",  { y: 36, opacity: 0, duration: 0.9, ease: "power3.out", stagger: 0.1, delay: 0.1 });
      gsap.from(".blog-featured",  { scale: 0.97, opacity: 0, duration: 1, ease: "power3.out", delay: 0.4 });
      gsap.from(".blog-card-item", {
        y: 45, opacity: 0, duration: 0.75, ease: "power3.out", stagger: 0.08,
  ...
  return (
    <div ref={rootRef} data-testid="blog-page">
      ...
      <section className="container-page pt-20 pb-14 max-w-3xl">
        <span className="blog-hero-txt label-eyebrow">The Journal</span>
        <h1 className="blog-hero-txt font-serif text-5xl lg:text-7xl text-brand-text font-medium tracking-tight mt-4 leading-[1.04]">
          Articles, insights &<br /><em className="text-brand-primary italic">patient education.</em>
        </h1>
        <p className="blog-hero-txt mt-6 text-lg text-brand-textSecondary leading-relaxed">
          Evidence-based reading on proctology — written to help you understand your symptoms, treatment options, and recovery.
        </p>
      </section>
          ...
          <Link to={`/blog/${featured.slug}`}
            ...
            <div className="lg:col-span-7 overflow-hidden rounded-2xl">
              <img src={featured.cover} alt={featured.title}
                ...
                onError={e => { e.target.style.background = "linear-gradient(135deg,#1A5B5E,#0E1B19)"; e.target.src = ""; }}
              />
            </div>
            <div className="lg:col-span-5">
              <div className="flex items-center gap-3 mb-3">
                <span className={`text-[10px] uppercase tracking-[0.2em] font-semibold px-2.5 py-1 rounded-full ${CATEGORY_COLORS[featured.category] || "text-brand-primary bg-brand-secondary"}`}>
                  {featured.category}
                </span>
              ...
              <p className="text-sm text-brand-textSecondary mt-4 leading-relaxed">{featured.excerpt}</p>
      ...
      {rest.length > 0 && (
        <section className="container-page pb-24 blog-grid grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {rest.map(p => (
            <Link key={p._id || p.slug} to={`/blog/${p.slug}`}
              ...
              <div className="overflow-hidden rounded-2xl mb-5 aspect-[4/3] bg-brand-subtle">
                <img src={p.cover} alt={p.title}
                  ...
                  onError={e => { e.target.style.background = "linear-gradient(135deg,#E8F4F4,#1A5B5E22)"; e.target.src = ""; }}
                />
              </div>
              <div className="flex items-center gap-3 mb-3">
                <span className={`text-[10px] uppercase tracking-[0.2em] font-semibold px-2.5 py-1 rounded-full ${CATEGORY_COLORS[p.category] || "text-brand-primary bg-brand-secondary"}`}>
                  {p.category}
                </span>
                <span className="flex items-center gap-1 text-xs text-brand-textMuted">
              ...
              <p className="text-sm text-brand-textSecondary mt-3 leading-relaxed line-clamp-2">{p.excerpt}</p>
            </Link>
          ))}
        </section>
      )}
